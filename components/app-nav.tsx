...
export function MobileBottomNav({ canSettings = false, canReports = false, canManageSubscription = false, canManageDebts = false }: { canSettings?: boolean; canReports?: boolean; canManageSubscription?: boolean; canManageDebts?: boolean }) {
  ...
  return <>
    ...
    {sheet && <><button type="button" aria-label="إغلاق القائمة" onClick={() => setSheet(null)} className="fixed inset-0 z-[60] bg-slate-950/45 backdrop-blur-[2px] lg:hidden" /><section className="fixed inset-x-0 bottom-0 z-[70] max-h-[78vh] overflow-hidden rounded-t-[28px] border-t border-slate-200 bg-white shadow-2xl lg:hidden" style={{ paddingBottom: "env(safe-area-inset-bottom)" }}><div className="mx-auto mt-2.5 h-1.5 w-12 rounded-full bg-slate-200" /><div className="flex items-center justify-between border-b border-slate-100 px-5 pb-4 pt-3"><div><p className="text-[15px] font-black text-slate-900">{sheet === "sales" ? "المبيعات" : sheet === "inventory" ? "المخزون" : "المزيد"}</p><p className="text-[11px] font-semibold text-slate-400">{sheet === "more" ? "الوصول السريع لبقية أقسام مسار" : "اختر الصفحة التي تريد فتحها"}</p></div><button type="button" onClick={() => setSheet(null)} className="flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-500"><X className="h-4 w-4" /></button></div><div className="max-h-[calc(78vh-92px)] overflow-y-auto px-4 py-4">{sheet === "sales" && <MobileGroup title="المبيعات" section={byKey("sales")} pathname={pathname} permissions={permissions} close={() => setSheet(null)} />}{sheet === "inventory" && <MobileGroup title="المخزون" section={byKey("inventory")} pathname={pathname} permissions={permissions} close={() => setSheet(null)} />}{sheet === "more" && <div className="space-y-5"><MobileDirectCard section={byKey("customers")} active={activeKey === "customers"} close={() => setSheet(null)} /><MobileGroup title="المالية" section={byKey("finance")} pathname={pathname} permissions={permissions} close={() => setSheet(null)} /><MobileGroup title="الإدارة والمساعدة" section={byKey("admin")} pathname={pathname} permissions={permissions} close={() => setSheet(null)} /></div>}</div></section></>}
  </>;
}
