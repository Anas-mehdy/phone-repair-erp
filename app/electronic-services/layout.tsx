...
export default function ElectronicServicesLayout({ children }: { children: ReactNode }) {
  return <div>
    <nav className="mb-4 flex flex-wrap items-center gap-2 rounded-2xl border border-slate-200/80 bg-white/90 p-2 shadow-sm backdrop-blur dark:border-slate-800 dark:bg-slate-950/85" aria-label="تنقل الخدمات الإلكترونية">
      ...
      <Link href="/electronic-services/reconcile" className="inline-flex min-h-9 items-center gap-1.5 rounded-xl px-3 text-[10px] font-black text-cyan-700 transition hover:bg-cyan-50 dark:text-cyan-300 dark:hover:bg-cyan-950/30"><Scale className="h-3.5 w-3.5" />مطابقة الأرصدة</Link>
      <Link href="/electronic-services/reports" className="inline-flex min-h-9 items-center gap-1.5 rounded-xl px-3 text-[10px] font-black text-violet-700 transition hover:bg-violet-50 dark:text-violet-300 dark:hover:bg-violet-950/30"><BarChart3 className="h-3.5 w-3.5" />التقارير</Link>
    </nav>
    {children}
  </div>;
}
