  ...
  return (
    <>
      ...
      <ReportSection
        ...
        description="سعر البيع والتكلفة والربح لكل قسم ضمن نفس الفترة المختارة."
      >
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          <PerformanceCard item={dashboard.departments.repairs} currency={currency} icon={Wrench} href={withRange("/repair-orders", query)} />
          <PerformanceCard item={dashboard.departments.pos} currency={currency} icon={CreditCard} href={withRange("/sales", query)} />
          <PerformanceCard item={dashboard.departments.transfers} currency={currency} icon={ArrowLeftRight} href={withRange("/transfers", query)} revenueLabel="القيمة الداخلة" costLabel="القيمة الخارجة" note="أصل مبلغ التحويل لا يدخل ضمن إجمالي المبيعات؛ الربح هو العمولة فقط." />
          <PerformanceCard item={dashboard.departments.electronic} currency={currency} icon={Zap} href={withRange("/electronic-services/reports", query)} />
          <PerformanceCard item={dashboard.departments.software} currency={currency} icon={Smartphone} href={withRange("/software-services", query)} />
