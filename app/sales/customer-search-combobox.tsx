...
import { searchCustomersForSaleAction } from "./actions";
...
export function CustomerSearchCombobox({
  ...
  value: string;
  selectedCustomer: SaleCustomerOption | null;
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      startTransition(async () => {
        try {
          const matches = await searchCustomersForSaleAction(trimmed);
  ...
  const displayValue = open ? query : selectedCustomer ? selectedCustomer.name : "";
    ...
    <div ref={rootRef} className="relative">
      ...
      {open ? (
        <div className="absolute z-50 mt-1.5 max-h-72 w-full overflow-y-auto rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl shadow-slate-200/60">
          {!query.trim() ? (
            <div className="px-3 py-5 text-center text-xs font-semibold text-slate-400">
              اكتب اسم العميل أو رقم هاتفه للبحث.
            </div>
          ) : results.length === 0 && !isPending ? (
            <div className="px-3 py-5 text-center text-xs font-semibold text-slate-400">
              لا يوجد عميل مطابق. يمكنك اختيار «عميل جديد».
            </div>
          ) : (
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  ...
                  <div className="min-w-0">
                    ...
                    {customer.phone ? (
                      <div className="mt-0.5 text-[10px] font-semibold text-slate-400 font-numeric" dir="ltr">
                        {customer.phone}
                      </div>
                    ) : null}
                  </div>
