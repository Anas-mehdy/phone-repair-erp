...
export default async function SaleDetailsPage({
  ...
  return (
    <div className="space-y-6">
      ...
      {query.invoiceError ? (
        <div className="rounded-2xl border border-rose-100 bg-rose-50/50 p-4 text-xs font-bold text-rose-600">
          {query.invoiceError}
        </div>
      ) : null}
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
        ...
        <div className="space-y-6">
          ...
          <div className="erp-section">
            <div className="border-b border-slate-100/60 pb-3 mb-4">
              <h3 className="font-bold text-slate-800 text-sm">بنود الفاتورة / المبيعات</h3>
            </div>
            <div className="overflow-hidden rounded-xl border border-slate-200/50">
              <div className="overflow-x-auto">
                <table className="erp-table min-w-[760px]">
                  <thead>
                    <tr>
                      <th className="text-slate-700">البند / الوصف</th>
                      <th className="text-slate-700">قطعة المخزون المرتبطة</th>
                      <th className="text-center text-slate-700">الكمية المباعة</th>
                      <th className="text-slate-700">سعر الوحدة</th>
                      <th className="text-slate-700">قيمة الخصم</th>
                      <th className="text-slate-700">الإجمالي الصافي</th>
                    </tr>
                  </thead>
                  <tbody>
                    {sale.items.map((item) => (
          ...
          <div className="erp-section">
            <div className="border-b border-slate-100/60 pb-3 mb-4">
              <h3 className="font-bold text-slate-800 text-sm">حركات المخزون المرتبطة</h3>
            </div>
            {sale.inventoryMovements.length === 0 ? (
              <p className="text-xs text-slate-400 font-medium py-6 text-center">
                لا توجد أية حركات مخزون مسجلة لهذه العملية.
              </p>
            ) : (
              <div className="overflow-hidden rounded-xl border border-slate-200/50">
                <div className="overflow-x-auto">
                  <table className="erp-table min-w-[680px]">
                    <thead>
                      <tr>
                        <th className="text-slate-700">القطعة</th>
        ...
        {/* Sidebar Column */}
        <div className="space-y-6">
          {/* Operations and Print/Share triggers */}
              ...
              {sale.status === SaleStatus.COMPLETED ? (
                <form action={cancelSaleAction}>
                  ...
                  <ConfirmSubmitButton
                    ...
                    message="هل تريد إلغاء عملية البيع وإرجاع الكميات للمخزون؟"
                  >
                    <Ban className="h-4 w-4 ml-1.5 shrink-0" aria-hidden="true" />
                    إلغاء عملية البيع بالكامل
                  </ConfirmSubmitButton>
                </form>
              ) : (
                <Button type="button" variant="outline" className="w-full border-rose-100 bg-rose-50/10 text-rose-600 font-bold rounded-xl h-11 text-xs justify-center cursor-not-allowed" disabled>
                  <Ban className="h-4 w-4 ml-1.5 shrink-0" aria-hidden="true" />
