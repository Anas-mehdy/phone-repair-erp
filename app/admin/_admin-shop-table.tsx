    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
        ...
        <div className="flex items-center gap-1.5 w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0">
          <button type="button" onClick={() => setActiveTab("all")} className={`px-3 py-1.5 rounded-xl text-xs font-bold transition whitespace-nowrap ${activeTab === "all" ? "bg-violet-600 text-white shadow-md shadow-violet-600/30" : "bg-slate-800/80 text-slate-400 hover:text-white"}`}>
            جميع المتاجر ({initialShops.length})
          </button>
          <button type="button" onClick={() => setActiveTab("active")} className={`px-3 py-1.5 rounded-xl text-xs font-bold transition whitespace-nowrap ${activeTab === "active" ? "bg-violet-600 text-white shadow-md shadow-violet-600/30" : "bg-slate-800/80 text-slate-400 hover:text-white"}`}>
            المتاجر النشطة ({initialShops.filter((s) => s.isActive).length})
          </button>
          <button type="button" onClick={() => setActiveTab("top")} className={`px-3 py-1.5 rounded-xl text-xs font-bold transition whitespace-nowrap ${activeTab === "top" ? "bg-violet-600 text-white shadow-md shadow-violet-600/30" : "bg-slate-800/80 text-slate-400 hover:text-white"}`}>
            الأعلى نشاطاً 🔥
          </button>
        </div>
      </div>
          ...
          <tbody className="divide-y divide-slate-800/60 font-medium">
            {sortedShops.length === 0 ? (
              ...
              sortedShops.map((shop) => {
                const normalizedWa = shop.phone ? normalizePhoneForWhatsApp(shop.phone, shop.currency) : null;
                const waUrl = normalizedWa ? `https://wa.me/${normalizedWa}?text=${encodeURIComponent(`مرحباً ${shop.owner?.name || shop.name}، معك إدارة منصة مصلح لإدارة مراكز الصيانة.`)}` : null;
                ...
                return (
                  <tr key={shop.id} className="hover:bg-slate-800/40 transition">
                      ...
                      <div className="flex items-start gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-1.5 flex-wrap">
                            ...
                            {!shop.isActive && <span className="text-[10px] bg-rose-500/20 text-rose-300 px-1.5 py-0.5 rounded border border-rose-500/30">مجمد</span>}
                          </div>
                          {shop.owner ? (
                    ...
                    <td className="py-3.5 px-4">
                      {shop.phone ? (
                        <div className="flex items-center gap-2">
                          <span className="font-numeric font-bold text-slate-300" dir="ltr">{shop.phone}</span>
                          {waUrl && (
                            <a href={waUrl} target="_blank" rel="noreferrer" title="فتح محادثة واتساب" className="flex h-6 w-6 items-center justify-center rounded-lg bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white transition">
                    ...
                    <td className="py-3.5 px-4 text-center">
                      <span className={`inline-flex items-center justify-center min-w-[28px] h-6 px-2 rounded-lg font-black font-numeric text-xs ${shop.counts.repairOrders > 0 ? "bg-teal-500/20 text-teal-300 border border-teal-500/30" : "bg-slate-800/60 text-slate-500"}`}>
                        {shop.counts.repairOrders}
                      </span>
                    </td>

                    <td className="py-3.5 px-4 text-center font-numeric font-bold text-slate-300">{shop.counts.customers}</td>
                    <td className="py-3.5 px-4 text-center font-numeric font-bold text-slate-400">{shop.counts.suppliers}</td>

                    <td className="py-3.5 px-4 font-numeric text-[11px] text-slate-400 whitespace-nowrap">
                      {new Date(shop.createdAt).toLocaleDateString("ar-EG", { day: "numeric", month: "short", year: "numeric" })}
                    </td>

                    <td className="py-3.5 px-4 whitespace-nowrap">
                      {shop.owner?.lastLoginAt ? (
                        <div className="space-y-0.5">
                          <div className="font-numeric text-[11px] font-bold text-slate-200">
                            {new Date(shop.owner.lastLoginAt).toLocaleDateString("ar-EG", { day: "numeric", month: "short", year: "numeric" })}
                          </div>
                          <div className="font-numeric text-[10px] text-slate-500">
                            {new Date(shop.owner.lastLoginAt).toLocaleTimeString("ar-EG", { hour: "2-digit", minute: "2-digit" })}
                          </div>
                        </div>
                      ) : (
                        <span className="inline-flex rounded-lg border border-slate-700 bg-slate-800/60 px-2 py-1 text-[10px] font-bold text-slate-500">
                          لم يسجل دخول بعد
                        </span>
                      )}
                    </td>
                    ...
                    <td className="py-3.5 px-4 text-left whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1.5">
                        {shop.owner && (
                          <button type="button" onClick={() => handleOpenReset(shop)} title="إعادة تعيين كلمة المرور" className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-slate-800 text-slate-300 hover:bg-violet-600 hover:text-white border border-slate-700 text-[11px] font-bold transition">
                            <KeyRound className="h-3.5 w-3.5" />
                            <span>كلمة السر</span>
                          </button>
                        )}
                        <button type="button" disabled={isImpersonating} onClick={() => handleImpersonate(shop.id)} title="الدخول كمتجر" className="flex items-center gap-1 px-2.5 py-1 rounded-lg bg-violet-500/10 text-violet-300 hover:bg-violet-600 hover:text-white border border-violet-500/20 text-[11px] font-bold transition">
                          <LogIn className="h-3.5 w-3.5" />
                          <span>دخول</span>
                        </button>
                        <button type="button" disabled={isTogglePending} onClick={() => handleToggleShop(shop.id, shop.isActive)} title={shop.isActive ? "تجميد المتجر" : "تنشيط المتجر"} className={`p-1 rounded-lg border transition ${shop.isActive ? "bg-slate-800/80 text-slate-400 hover:bg-rose-500/20 hover:text-rose-400 border-slate-700" : "bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500 hover:text-white border-emerald-500/30"}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-xs animate-in fade-in duration-200">
          <div className="relative w-full max-w-md bg-slate-900 rounded-3xl border border-slate-800 shadow-2xl p-6 text-right">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800">
              ...
              <button type="button" onClick={() => setResetModalShop(null)} className="rounded-full p-1.5 text-slate-400 hover:text-white hover:bg-slate-800 transition">
                <X className="h-4 w-4" />
              </button>
            </div>

            {resetResult && (
              <div className={`mt-4 p-3 rounded-xl text-xs font-bold flex items-center gap-2 ${resetResult.success ? "bg-emerald-500/20 text-emerald-300 border border-emerald-500/30" : "bg-rose-500/20 text-rose-300 border border-rose-500/30"}`}>
            ...
            <form onSubmit={handleResetSubmit} className="mt-4 space-y-4">
              <div className="p-3 bg-slate-950/80 rounded-xl border border-slate-800 text-xs space-y-1">
                <div className="text-slate-400">صاحب المتجر: <span className="font-bold text-white">{resetModalShop.owner.name}</span></div>
                <div className="text-slate-400">البريد الإلكتروني: <span className="font-mono text-violet-300">{resetModalShop.owner.email}</span></div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="text-xs font-extrabold text-slate-300">كلمة المرور الجديدة:</label>
                  <button type="button" onClick={generateRandomPassword} className="flex items-center gap-1 text-[11px] font-bold text-violet-400 hover:text-violet-300">
                    <Sparkles className="h-3 w-3" />
                    توليد كلمة عشوائية
                  </button>
                </div>
                <input type="text" required minLength={6} value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-950 py-2.5 px-3 text-sm font-mono text-white focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500" dir="ltr" />
              </div>

              <div className="flex items-center gap-2 pt-2">
                <Button type="submit" disabled={isResetPending} className="flex-1 h-11 bg-violet-600 hover:bg-violet-500 text-white font-bold text-xs rounded-xl">
                  {isResetPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "حفظ كلمة المرور الجديدة"}
                </Button>
                <Button type="button" variant="outline" onClick={handleCopyCredentials} className="h-11 px-3 bg-slate-800 hover:bg-slate-700 border-slate-700 text-slate-200 text-xs font-bold rounded-xl">
