...
export default async function CashDrawerPage({ searchParams }: CashDrawerPageProps) {
  ...
  const canManageLiquidity = can(auth, "expenses:manage");
  const [wallets, drawer, bankAccounts] = await Promise.all([
    canManageLiquidity ? financialTransferService.listWallets(auth.shop.id) : Promise.resolve([]),
    cashDrawerService.getAuditSnapshot(auth.shop.id, 150),
    canManageLiquidity ? bankAccountService.listAccounts(auth.shop.id) : Promise.resolve([]),
  ]);
    ...
    {canManageLiquidity ? (
      <section className="grid gap-5 xl:grid-cols-2">
        ...
        <CashLiquidityTransferForm
          ...
          currency={currency}
          canManageBank={canManageLiquidity}
        />
      </section>
    ) : (
      <div className="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-xs font-bold leading-6 text-slate-600">
        لديك صلاحية عرض الدرج وسجل حركاته فقط. تعديل الرصيد أو إضافة حركة يدوية أو نقل السيولة يحتاج صلاحية إدارة المصروفات والمالية.
      </div>
    )}
