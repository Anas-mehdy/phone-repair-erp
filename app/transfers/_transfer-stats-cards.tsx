...
export function TransferStatsCards({ stats, wallets, operations, currency }: { stats: Stats; wallets: WalletItem[]; operations: OperationItem[]; currency: string }) {
  ...
  return <>
    ...
    {metric ? <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/55 p-4 backdrop-blur-[3px]" onMouseDown={(event) => { if (event.currentTarget === event.target) setMetric(null); }}>
      <div className="w-full max-w-3xl overflow-hidden rounded-[24px] border border-teal-100 bg-white shadow-2xl" role="dialog" aria-modal="true">
        ...
        {metric === "balances" ? <BalancesView wallets={wallets} currency={currency} /> : <div className="p-5 sm:p-6"><div className="mb-5 flex flex-wrap gap-2">{([["service","حسب المحفظة"],["user","حسب المستخدم"],["direction","حسب الاتجاه"],["all",`كل العمليات (${filtered.length})`]] as Array<[TabKey,string]>).map(([key,label]) => <button key={key} type="button" onClick={() => setTab(key)} className={`rounded-xl px-3.5 py-2 text-[10px] font-black transition ${tab === key ? "bg-teal-600 text-white shadow-md shadow-teal-600/15" : "border border-slate-200 bg-slate-50 text-slate-500 hover:bg-white"}`}>{label}</button>)}</div>{filtered.length === 0 ? <div className="py-14 text-center text-xs font-bold text-slate-400">لا توجد بيانات اليوم.</div> : tab === "all" ? <OperationsTable rows={filtered} metric={metric} currency={currency} /> : <GroupedTable rows={grouped} metric={metric} currency={currency} />}</div>}
      </div>
    </div> : null}
  </>;
}
...
function OperationsTable({ rows, metric, currency }: { rows: OperationItem[]; metric: MetricKey; currency: string }) { return <div className="max-h-[420px] overflow-auto rounded-2xl border border-slate-200"><table className="min-w-[760px] w-full text-right text-xs"><thead className="sticky top-0 bg-slate-50 text-slate-500"><tr><th className="px-4 py-3">المحفظة</th><th className="px-4 py-3">الاتجاه</th><th className="px-4 py-3">المستخدم</th><th className="px-4 py-3">العميل</th><th className="px-4 py-3">المبلغ</th><th className="px-4 py-3">العمولة</th><th className="px-4 py-3">الوقت</th></tr></thead><tbody className="divide-y divide-slate-100">{rows.map((row) => <tr key={row.id}><td className="px-4 py-3 font-black text-slate-800">{row.walletName}</td><td className="px-4 py-3 font-bold text-slate-600">{directionLabel[row.operationType]}</td><td className="px-4 py-3 font-bold text-slate-600">{row.userName}</td><td className="px-4 py-3 font-bold text-slate-500">{row.customerName || "—"}</td><td className="px-4 py-3 font-numeric font-black">{money(row.amount, currency)}</td><td className={`px-4 py-3 font-numeric font-black ${metric === "commission" ? "text-amber-700" : "text-teal-700"}`}>{money(row.commission, currency)}</td><td className="px-4 py-3 font-numeric font-bold text-slate-400">{new Date(row.createdAt).toLocaleTimeString("ar", { hour: "2-digit", minute: "2-digit" })}</td></tr>)}</tbody></table></div>; }
