...
import { Crown, Flame, Infinity, MessageCircle, Sparkles, Wrench } from "lucide-react";
...
export function LifetimePlanCard({
  ...
  const maintenanceConfigured = price?.annualMaintenanceAmount != null && price.annualMaintenanceAmount > 0;
  const available = Boolean(isActive && !soldOut && price && maintenanceConfigured);
  ...
  const maintenanceText = price && maintenanceConfigured
    ? ` ورسوم صيانة وتحديث سنوية ${price.annualMaintenanceAmount} ${price.currencyCode} تبدأ من السنة الثانية، والسنة الأولى مجانية.`
    : "";
  const message = price
    ? `مرحباً، أريد الاشتراك في خطة مسار مدى الحياة لمتجري (${shopName}) بسعر (${price.amount} ${price.currencyCode}) ضمن العرض المحدود لأول ${totalEligible} مشترك.${maintenanceText}`
  ...
  function trackLifetimeUpgrade() {
    if (!price || !maintenanceConfigured) return;
    captureClientEvent(ANALYTICS_EVENTS.UPGRADE_CLICKED, {
      destination: "whatsapp",
      interval: "lifetime",
      price: price.amount,
      annual_maintenance_price: price.annualMaintenanceAmount,
      annual_maintenance_first_year_free: true,
      ...
      <div className="grid gap-6 p-6 sm:p-8 lg:grid-cols-[1fr_360px] lg:items-center">
        <div className="space-y-4">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <div>
              ...
              <p className="mt-1 text-xs font-semibold leading-6 text-slate-600">دفعة ترخيص واحدة تمنحك استخدام مسار مدى الحياة. رسوم الصيانة والتحديثات السنوية منفصلة وتبدأ من السنة الثانية فقط.</p>
            </div>
          </div>

          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            <div className="rounded-2xl border border-amber-200 bg-white/80 p-3"><Infinity className="mb-1 h-4 w-4 text-amber-600" /><div className="text-xs font-black text-slate-900">استخدام مدى الحياة</div><div className="mt-0.5 text-[10px] font-semibold text-slate-500">ترخيص الاستخدام لا ينتهي</div></div>
            ...
            <div className="rounded-2xl border border-emerald-200 bg-emerald-50/80 p-3"><Wrench className="mb-1 h-4 w-4 text-emerald-600" /><div className="text-xs font-black text-slate-900">السنة الأولى مجانية</div><div className="mt-0.5 text-[10px] font-semibold text-slate-500">صيانة وتحديثات دون رسم سنوي</div></div>
        ...
        <div className="rounded-3xl border border-slate-800 bg-slate-950 p-5 text-white shadow-xl">
          <div className="text-center">
            <span className="text-[11px] font-black text-amber-300">سعر ترخيص مدى الحياة</span>
            {price ? (
          ...
          <div className="mt-4 rounded-2xl border border-emerald-500/20 bg-emerald-500/10 p-3 text-center">
            <div className="text-[10px] font-black text-emerald-300">الصيانة والتحديثات السنوية</div>
            {price && maintenanceConfigured ? (
              <>
                <div className="mt-1 font-numeric text-lg font-black text-white">{price.annualMaintenanceAmount!.toLocaleString()} {price.currencyCode} <span className="text-xs font-bold text-slate-400">/ سنة</span></div>
                <div className="mt-1 text-[10px] font-bold text-slate-300">السنة الأولى مجانية — يبدأ الدفع من السنة الثانية</div>
              </>
            ) : (
              <div className="mt-1 text-[10px] font-bold text-amber-200">الرسم السنوي لم يُحدد لهذه الدولة بعد</div>
            )}
          </div>
          ...
          <Button asChild={available} disabled={!available} className="mt-4 h-12 w-full rounded-2xl bg-amber-500 font-black text-slate-950 hover:bg-amber-400 disabled:cursor-not-allowed disabled:opacity-50">
            {available ? (
              ...
              <span>{soldOut ? "اكتمل العدد المخصص" : isActive && price ? "بانتظار تحديد الرسم السنوي" : "الخطة غير متاحة حالياً"}</span>
            )}
          </Button>
        </div>
      </div>
