...
async function assertActivePartnerById(partnerId: string) {
  const rows = await prisma.$queryRaw<Array<{ id: string; code: string; name: string }>>`
    ...
    WHERE "id" = ${partnerId}::uuid AND "status" = 'ACTIVE'::"PartnerStatus" AND "deletedAt" IS NULL
    LIMIT 1
  `;
  ...
  try {
    await prisma.$executeRaw`
      INSERT INTO "PartnerClientInvitation" (
        ...
        ${id}::uuid, ${session.partnerId}::uuid, ${clientName}, ${email}, ${hash}, 'PENDING', ${expiresAt}, NOW(), NOW()
      )
    `;
  } catch {
    throw new Error("يوجد بالفعل رابط دعوة فعال لهذا البريد. ألغِه أو انتظر انتهاءه.");
  }
...
export async function listPartnerClientInvitations() {
  ...
  return prisma.$queryRaw<Array<{
    id: string; clientName: string; email: string; status: string; expiresAt: Date; usedAt: Date | null; createdAt: Date;
  }>>`
    SELECT "id", "clientName", "email",
      CASE WHEN "status" = 'PENDING' AND "expiresAt" <= NOW() THEN 'EXPIRED' ELSE "status" END AS "status",
      "expiresAt", "usedAt", "createdAt"
    FROM "PartnerClientInvitation"
...
export async function getInvitationPreview(token: string) {
  ...
  const rows = await prisma.$queryRaw<Array<{
    partnerId: string; partnerName: string; partnerCode: string; clientName: string; email: string; expiresAt: Date; status: string;
  }>>`
    SELECT i."partnerId", p."name" AS "partnerName", p."code" AS "partnerCode",
      i."clientName", i."email", i."expiresAt", i."status"
    FROM "PartnerClientInvitation" i JOIN "Partner" p ON p."id" = i."partnerId"
    WHERE i."tokenHash" = ${hash} AND p."status" = 'ACTIVE'::"PartnerStatus" AND p."deletedAt" IS NULL
    LIMIT 1
  `;
  const row = rows[0];
...
export async function getPublicPartnerPreview(code: string) {
  ...
  const rows = await prisma.$queryRaw<Array<{ partnerId: string; partnerName: string; partnerCode: string }>>`
    SELECT "id" AS "partnerId", "name" AS "partnerName", "code" AS "partnerCode"
    FROM "Partner"
    WHERE upper("code") = ${normalized} AND "status" = 'ACTIVE'::"PartnerStatus" AND "deletedAt" IS NULL
    LIMIT 1
  `;
  return rows[0] ?? null;
}

export type PartnerClientRegistrationInput = {
  ownerName: string;
  email: string;
  password: string;
  shopName: string;
...
async function createPartnerManagedShop(partnerId: string, input: PartnerClientRegistrationInput) {
  await assertActivePartnerById(partnerId);
  ...
  const result = await prisma.$transaction(async (tx) => {
    const shop = await tx.shop.create({
      data: {
        name: v.shopName, phone: v.phone, countryCode: v.countryCode, currency: v.currency,
        address: v.address, taxRate: 15,
        subscription: { create: { plan: SubscriptionPlan.PROFESSIONAL, status: SubscriptionStatus.TRIALING, trialStartedAt, trialEndsAt } },
      },
    });
...
export async function registerFromPartnerInvitation(token: string, input: PartnerClientRegistrationInput) {
  ...
  return prisma.$transaction(async (tx) => {
    const rows = await tx.$queryRaw<Array<{ id: string; partnerId: string; clientName: string; email: string; expiresAt: Date; status: string }>>`
      SELECT "id", "partnerId", "clientName", "email", "expiresAt", "status"
      FROM "PartnerClientInvitation" WHERE "tokenHash" = ${hash} FOR UPDATE
    `;
    const invite = rows[0];
    if (!invite || invite.status !== "PENDING" || invite.expiresAt <= new Date()) throw new Error("رابط الدعوة غير صالح أو منتهي.");
    if (normalizeEmail(input.email) !== invite.email) throw new Error("البريد الإلكتروني لا يطابق الدعوة.");

    const partnerRows = await tx.$queryRaw<Array<{ id: string }>>`
      SELECT "id" FROM "Partner" WHERE "id" = ${invite.partnerId}::uuid AND "status" = 'ACTIVE'::"PartnerStatus" AND "deletedAt" IS NULL LIMIT 1
    `;
