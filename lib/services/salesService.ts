...
export type CreateSaleInput = {
  ...
  paymentDestination?: Exclude<MoneyAccountDestination, "OTHER">;
  walletId?: string;
...
async function resolveCustomerForSale(tx: Prisma.TransactionClient, shopId: string, input: Pick<CreateSaleInput, "customerId" | "customerName" | "customerPhone">) {
  if (input.customerId) {
    const selectedCustomer = await tx.customer.findFirst({ where: { id: input.customerId, shopId, deletedAt: null } });
    if (!selectedCustomer) throw new Error("العميل المحدد غير موجود أو لا يتبع هذا المتجر.");
  ...
  const phoneNormalized = customerPhone ? normalizePhone(customerPhone) : null;
  if (customerPhone) {
    const existingCustomer = await tx.customer.findFirst({ where: { shopId, deletedAt: null, OR: [...(phoneNormalized ? [{ phoneNormalized }] : []), { phone: customerPhone }] }, select: { id: true, name: true, phone: true } });
    if (existingCustomer) throw new Error(`يوجد عميل مسجل بهذا الرقم باسم «${existingCustomer.name}». اختر «عميل موجود» وابحث عنه بدلاً من إنشاء عميل جديد.`);
  }
...
export async function generateReceiptNumber(shopId: string) {
  ...
  const count = await prisma.sale.count({ where: { shopId, receiptNumber: { startsWith: prefix } } });
  for (let offset = 1; offset <= 50; offset += 1) {
    const receiptNumber = `${prefix}${String(count + offset).padStart(4, "0")}`;
    const existing = await prisma.sale.findUnique({ where: { shopId_receiptNumber: { shopId, receiptNumber } }, select: { id: true } });
...
export async function listSales(shopId: string, filters: SaleFilters = {}) {
  ...
  return prisma.sale.findMany({
    where: { shopId, deletedAt: null, ...(status ? { status } : {}), ...(search ? { OR: [{ receiptNumber: { contains: search, mode: "insensitive" } }, { customer: { is: { OR: [{ name: { contains: search, mode: "insensitive" } }, { phone: { contains: search, mode: "insensitive" } }, { phoneNormalized: { contains: search, mode: "insensitive" } }] } } }] } : {}) },
...
export async function createSale(shopId: string, createdByUserId: string | null, input: CreateSaleInput) {
  ...
  const subtotal = input.items.reduce((sum, item) => sum.add(decimal(item.unitPrice).mul(item.quantity)), new Prisma.Decimal(0));
  const discountTotal = input.items.reduce((sum, item) => sum.add(decimal(item.discountTotal ?? "0")), new Prisma.Decimal(0));
  const taxTotal = new Prisma.Decimal(0);
  const total = subtotal.sub(discountTotal).add(taxTotal);
  if (total.lt(0)) throw new Error("إجمالي البيع غير صحيح.");

  const paymentDestination = input.paymentDestination ?? "DRAWER";
  const amountReceived = input.amountReceived?.trim() ? decimal(input.amountReceived) : total;
  if (amountReceived.lt(total)) throw new Error("المبلغ المستلم أقل من إجمالي البيع. استخدم فاتورة/دين إذا كان هناك مبلغ متبقٍ.");
  const changeAmount = amountReceived.sub(total);
  const changeDestination = input.changeDestination ?? "DRAWER";

  await moneyAccountService.prepareMoneyAccounts(shopId, paymentDestination);
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const itemInput of input.items) {
      ...
      let description = itemInput.description.trim();
      let inventoryItem: Awaited<ReturnType<typeof tx.inventoryItem.findFirst>> | null = null;
      if (inventoryItemId) {
        inventoryItem = await tx.inventoryItem.findFirst({ where: { id: inventoryItemId, shopId, deletedAt: null } });
        if (!inventoryItem) throw new Error("قطعة مخزون غير موجودة أو لا تتبع هذا المتجر.");
        if (inventoryItem.quantity < itemInput.quantity) throw new Error(`الكمية غير كافية للقطعة: ${inventoryItem.name}`);
      ...
      const lineDiscount = decimal(itemInput.discountTotal ?? "0");
      const lineTotal = unitPriceSnapshot.mul(itemInput.quantity).sub(lineDiscount);
    ...
    if (changeAmount.gt(0)) {
      await moneyAccountService.applyOutgoingMoneyTx(tx, shopId, createdByUserId, { destination: changeDestination, walletId: input.changeWalletId, amount: changeAmount, reference: sale.receiptNumber, description: `إرجاع باقي للعميل من بيع ${sale.receiptNumber}` });
    }
    return sale;
...
export async function cancelSale(shopId: string, saleId: string, createdByUserId: string | null) {
  return prisma.$transaction(async (tx) => {
    const sale = await tx.sale.findFirst({ where: { id: saleId, shopId, deletedAt: null }, include: { items: true } });
    if (!sale) throw new Error("عملية البيع غير موجودة.");
    if (sale.status !== SaleStatus.COMPLETED) return sale;
